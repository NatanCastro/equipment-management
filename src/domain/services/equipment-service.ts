...
export const newEquipmentService: NewEquipmentService = (
	...
	return {
		...
		findOneEquipment: async (dto) => {
			...
			if (isErr(result)) {
				return err(
					`Não foi possível buscar o equipamento com o id ${dto.id}: ${result.err.message}`
				)
			}
			return result
		},
		...
		updateEquipment: async (dto: UpdateEquipmentDTO) => {
			...
			if (isErr(result)) {
				return err(
					`Não foi possível atualizar o equipamento com o id ${dto.id}: ${result.err.message}`
				)
			}
			return result
		},
		deleteEquipment: async (dto: DeleteEquipmentDTO) => {
			...
			if (isErr(result)) {
				return err(
					`Não foi possível excluir o equipamento com o id ${dto.id}: ${result.err.message}`
				)
			}
			return result
		}
