import type {
	FindEquipmentLocationsDTO,
	CreateEquipmentLocationDTO,
	UpdateEquipmentLocationDTO,
	DeleteEquipmentLocationDTO,
	FindOneEquipmentLocationDTO
} from "@/data/dtos"
...
export interface EquipmentLocationService {
	findEquipmentLocations: (
		dto: FindEquipmentLocationsDTO
	) => Promise<Result<EquipmentLocation[], string>>
	findOneEquipmentLocation: (
		dto: FindOneEquipmentLocationDTO
	) => Promise<Result<EquipmentLocation, string>>
	createEquipmentLocation: (
		dto: CreateEquipmentLocationDTO
	) => Promise<Result<EquipmentLocation, string>>
...
type EquipmentLocationServiceFactory = (
	repository: EquipmentLocationRepository
) => EquipmentLocationService

export const equipmentLocationServiceFactory: EquipmentLocationServiceFactory =
	(repository: EquipmentLocationRepository) => ({
		findEquipmentLocations: async (dto) => {
			const result = await repository.findEquipmentLocations(dto)
			if (isErr(result)) {
				return err(
					`Erro ao buscar locais de equipamento: ${result.err.message}`
				)
			}
			return result
		},
		...
		updateEquipmentLocation: async (dto: UpdateEquipmentLocationDTO) => {
			...
			if (isErr(result)) {
				return err(
					`Erro ao atualizar local de equipamento com o id ${dto.id}: ${result.err.message}`
				)
			}
			return result
		},
		deleteEquipmentLocation: async (dto: DeleteEquipmentLocationDTO) => {
			...
			if (isErr(result)) {
				return err(
					`Erro ao excluir local de equipamento com o id ${dto.id}: ${result.err.message}`
				)
			}
			return result
		}
